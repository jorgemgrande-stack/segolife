...
| **`communities`** | La unidad real de tenant de Segolife (SEGOLIFE IE, SEGOLIFE UVA, futuros campus). Es a lo que TODO lo demás se ancla: contenido, eventos, promociones, idioma por defecto, branding. **Sin columna `university_id`** — ver `community_universities` justo debajo. | La tabla `organizations` (id, name, slug, status, ownerUserId) ya existe en el schema heredado y es el punto de partida correcto en **forma** — pero está completamente desconectada (0 tablas de negocio la referencian hoy). Se implementó como tabla independiente `communities` (no se tocó `organizations`, que sigue intacta), con `default_locale`, `available_locales` (JSON) y `status`. |
| **`community_universities`** (tabla puente M2M) | A qué universidad(es) está asociada una comunidad. **Corrección de diseño (revisión previa al commit de Fase 1B):** la primera implementación usaba `communities.university_id` como FK directa nullable (relación 1:N — cada comunidad, como mucho, UNA universidad), lo cual contradecía el requisito explícito de que "una comunidad eventualmente pueda abarcar más de una universidad". Sustituido por esta tabla puente antes del primer commit del schema. | Nueva. Incluye `unique(community_id, university_id)` para no duplicar el mismo enlace. |
...
| **`user_communities`** (tabla puente M2M) | A qué comunidad(es) pertenece un usuario, con `role_in_community` y `joined_at`. `unique(user_id, community_id)` real en MySQL — implementada en Fase 1B, verificada contra la BD real (ver `docs/SEGOLIFE_ROADMAP.md`). | No existe heredado. Modelo M2M explícito en vez de columna única `community_id` en `users`, porque aunque el caso normal es 1 estudiante = 1 comunidad (por dominio de email), la relación M2M no cierra la puerta a: un admin global sin comunidad fija, un estudiante que se mueve de universidad, o —a futuro— comunidades no ligadas a una única universidad. |
