...
| LA FINCA CLUB | 🟠 AMBER | Cuenta, staff, catálogo (33 productos). **Actualizado 2026-08-20** — integración Fourvenues dada de alta (credenciales guardadas), pero `enabled=false`/`sync_enabled=false`/`environment=sandbox` — nunca ha sincronizado (0 sync runs, 0 eventos, 0 mappings). Test de conexión real (solo lectura, vía el mecanismo ya autorizado del admin) → `GET /events/ → HTTP 404` contra el entorno sandbox configurado; las otras 3 integraciones activas usan `environment=production`, lo que sugiere que el API key guardado es de producción pero el selector de entorno se quedó en el valor por defecto. Ver `FOURVENUES_LA_FINCA_PARITY_AUDIT_REPORT` para el detalle completo — arquitectura confirmada 100% genérica (0 bugs de mapping/scoping/scheduler), 1 bug real corregido (filtro de venue hardcodeado en `/admin/students/historical`, no relacionado con el fallo de conexión). Decisión pendiente del admin: confirmar `environment=production` y activar `enabled`/`sync_enabled` desde `/admin/integrations` cuando corresponda — no se ha tocado, es una decisión de configuración real, no un bug de código |
...
| FOURVENUES | 🟢 GREEN | 3/3 integraciones activas operativas (Casanova, Limoncello, Tía Felisa) + 1 dada de alta pendiente de activar (La Finca Club, ver fila propia arriba — `environment`/`enabled` pendientes de confirmar por el admin, sin bug de código) |
