...
| LA FINCA CLUB | 🟢 GREEN | Cuenta, staff, catálogo (33 productos), Fourvenues conectado. **Actualizado 2026-08-21 (OPS-01)** — activada en producción: probado de forma inequívoca y solo lectura que las credenciales guardadas son `ik_live_...` (estilo producción) y responden `200 {"success":true}` contra la API de producción de Fourvenues (el `environment=sandbox` guardado era simplemente el campo equivocado). Cambiado `environment=production`, `enabled=true`, `sync_enabled=true`, `status=connected` — `loyalty_enabled` queda deliberadamente en `false` (mismo patrón de activación gradual que las otras 3 venues en su día). Primer sync real (automático, recogido por el scheduler sin ningún cambio de código — 0 hardcode) importó 2 eventos reales (`LA FINCA CLUB VOL.04`/`VOL.05`, ambos `sourcePublicationStatus=unpublished` — drafts en Fourvenues, correctamente ocultos al Student, visibles en Admin Events con el badge "Borrador Fourvenues") y 6 tipos de entrada. Segundo sync (idempotencia) confirmado sin duplicados (0 creados, 2 actualizados, mismos 8 mappings). Comunidades IE+UVA sin cambios. 0 movimientos SegoTokens, 0 notificaciones a Students. Ver `OPS-01 FOURVENUES LA FINCA PRODUCTION ACTIVATION FINAL REPORT` para el detalle completo |
...
| FOURVENUES | 🟢 GREEN | 4/4 integraciones activas operativas en producción (Casanova, Limoncello, Tía Felisa, La Finca Club — activada 2026-08-21, ver fila propia arriba) |
