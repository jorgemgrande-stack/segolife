...
async function main() {
  ...
  async function del(label: string, sqlQuery: ReturnType<typeof drizzleSql>, idsForManifest: number[] = []) {
    if (dryRun) {
      console.log(`[dry-run] ${label}: ${idsForManifest.length} fila(s) se borrarían`);
      return idsForManifest.length;
    }
  ...
  await del("consumption_qr_codes (source_type)", drizzleSql`DELETE FROM consumption_qr_codes WHERE source_type = ${SIM_TAG}`);

  // A2. commerce_transactions (laguna real del rollback oficial — vía loyalty_ledger_id)
  if (simLedgerIds.length > 0) {
    await del("commerce_transactions (loyalty_ledger_id)", drizzleSql`DELETE FROM commerce_transactions WHERE loyalty_ledger_id IN (${drizzleSql.join(simLedgerIds, drizzleSql`,`)})`);
  }

  // A3. notifications, user_benefits (cubiertas por metadata.simulation, igual que el rollback oficial)
  await del("notifications (metadata.simulation)", drizzleSql`DELETE FROM notifications WHERE JSON_EXTRACT(metadata, '$.simulation') = ${SIM_TAG}`);
  await del("user_benefits (metadata.simulation)", drizzleSql`DELETE FROM user_benefits WHERE JSON_EXTRACT(metadata, '$.simulation') = ${SIM_TAG}`);

  // A4. token_ledger
  await del("token_ledger (metadata.simulation)", drizzleSql`DELETE FROM token_ledger WHERE JSON_EXTRACT(metadata, '$.simulation') = ${SIM_TAG}`);
  ...
  for (const uid of [4, 5, 6]) {
    const [rows] = await db.execute(drizzleSql`
      ...
      FROM token_ledger WHERE user_id = ${uid}
    `);
    const { earned, spent } = (rows as unknown as Array<{ earned: number; spent: number }>)[0];
    const balance = Number(earned) - Number(spent);
    console.log(`  wallet user_id=${uid} recalculada: balance=${balance} lifetimeEarned=${earned} lifetimeSpent=${spent}`);
    if (!dryRun) {
  ...
  await del("ticket_payments (metadata.simulation)", drizzleSql`DELETE FROM ticket_payments WHERE JSON_EXTRACT(metadata, '$.simulation') = ${SIM_TAG}`);

  if (simOrderIds.length > 0) {
    await del("ticket_order_items (huérfanos de pedidos de simulación)", drizzleSql`DELETE FROM ticket_order_items WHERE order_id IN (${drizzleSql.join(simOrderIds, drizzleSql`,`)})`);
    ...
    if (simTicketIds.length > 0) {
      await del("event_attendance (huérfanas de tickets de simulación)", drizzleSql`DELETE FROM event_attendance WHERE ticket_id IN (${drizzleSql.join(simTicketIds, drizzleSql`,`)})`);
      await del("event_tickets (huérfanos de pedidos de simulación)", drizzleSql`DELETE FROM event_tickets WHERE id IN (${drizzleSql.join(simTicketIds, drizzleSql`,`)})`);
    }

    await del("ticket_orders (metadata.simulation)", drizzleSql`DELETE FROM ticket_orders WHERE id IN (${drizzleSql.join(simOrderIds, drizzleSql`,`)})`);
  }

  // A7. venue_products
  await del("venue_products (metadata.simulation)", drizzleSql`DELETE FROM venue_products WHERE JSON_EXTRACT(metadata, '$.simulation') = ${SIM_TAG}`);
  ...
  await del(
    ...
    drizzleSql`DELETE FROM student_login_events WHERE user_id IN (4,5,6) AND occurred_at BETWEEN '2025-09-01' AND '2026-07-31 23:59:59'`
  );
  ...
  await del("sales_channels (30 eventos históricos)", drizzleSql`DELETE FROM sales_channels WHERE event_id IN (${drizzleSql.join(HISTORICAL_EVENT_IDS, drizzleSql`,`)})`);
  await del("event_ticket_types (30 eventos históricos)", drizzleSql`DELETE FROM event_ticket_types WHERE event_id IN (${drizzleSql.join(HISTORICAL_EVENT_IDS, drizzleSql`,`)})`);
  await del("community_events (30 eventos históricos)", drizzleSql`DELETE FROM community_events WHERE event_id IN (${drizzleSql.join(HISTORICAL_EVENT_IDS, drizzleSql`,`)})`);
  await del("events (30 eventos históricos)", drizzleSql`DELETE FROM events WHERE id IN (${drizzleSql.join(HISTORICAL_EVENT_IDS, drizzleSql`,`)})`, HISTORICAL_EVENT_IDS);
  ...
  await del("ticket_order_items (evento QA #10)", drizzleSql`DELETE FROM ticket_order_items WHERE order_id IN (${drizzleSql.join(EVENT10_ORDER_IDS, drizzleSql`,`)})`);
  await del("ticket_orders (evento QA #10, abandonados)", drizzleSql`DELETE FROM ticket_orders WHERE id IN (${drizzleSql.join(EVENT10_ORDER_IDS, drizzleSql`,`)})`, EVENT10_ORDER_IDS);
  ...
  await del("sales_channels (manifest QA, incl. #12 inerte)", drizzleSql`DELETE FROM sales_channels WHERE event_id IN (${drizzleSql.join(QA_MANIFEST_EVENT_IDS, drizzleSql`,`)})`);
  await del("event_ticket_types (manifest QA)", drizzleSql`DELETE FROM event_ticket_types WHERE event_id IN (${drizzleSql.join(QA_MANIFEST_EVENT_IDS, drizzleSql`,`)})`);
  await del("community_events (manifest QA)", drizzleSql`DELETE FROM community_events WHERE event_id IN (${drizzleSql.join(QA_MANIFEST_EVENT_IDS, drizzleSql`,`)})`);
  await del("events (manifest QA, 10 eventos)", drizzleSql`DELETE FROM events WHERE id IN (${drizzleSql.join(QA_MANIFEST_EVENT_IDS, drizzleSql`,`)})`, QA_MANIFEST_EVENT_IDS);
